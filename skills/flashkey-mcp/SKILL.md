...
FlashKey FK-01 是双芯片设备，插上后系统会出现**两个**串口：

| 端口 | VID/PID | role | 用途 |
|------|---------|------|------|
| `/dev/ttyACM0` | 1A86:FE0D | `fk_control` | **FK-01 主控** — 仅 MCP 内部使用，GPIO/认证 |
| `/dev/ttyUSB0` | 1A86:7523 | `fk_flash` | **CH340C 烧录口** — `flashkey_flash()` 和 `flashkey_log()` 用这个 |

**调用 `flashkey_flash()` 或 `flashkey_log()` 前，必须先调 `flashkey_list_ports()`，选择 `role=fk_flash` 的端口。永远不要把 `fk_control` 端口传给烧录/日志函数。**
