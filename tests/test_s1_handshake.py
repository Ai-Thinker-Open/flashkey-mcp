#!/usr/bin/env python3
"""FlashKey FK-01 S1 test: PING + Auth handshake + verify PB11 trigger"""
from flashkey_mcp.colors import green, red, yellow, cyan, gray, bold, bold_yellow
import serial, time, os

def crc8(data):
    crc = 0
    for b in data:
        crc ^= b
        for _ in range(8):
            if crc & 0x80:
                crc = ((crc << 1) ^ 0x31) & 0xFF
            else:
                crc = (crc << 1) & 0xFF
    return crc

def build_frame(cmd, data=b""):
    body = bytes([len(data) + 2, cmd]) + data
    return bytes([0x7E]) + body + bytes([crc8(body), 0x7F])

class FrameParser:
    SOF = 0x7E
    EOF = 0x7F
    STATE_IDLE, STATE_SOF, STATE_LEN, STATE_CMD, STATE_DATA, STATE_CRC = range(6)

    def __init__(self):
        self.reset()

    def reset(self):
        self.state = self.STATE_IDLE
        self.buf = bytearray()
        self.frame_len = 0
        self.data_len = 0

    def feed(self, byte):
        if self.state == self.STATE_IDLE:
            if byte == self.SOF:
                self.state = self.STATE_SOF
                self.buf = bytearray()
        elif self.state == self.STATE_SOF:
            self.frame_len = byte
            self.buf.append(byte)
            self.state = self.STATE_LEN
        elif self.state == self.STATE_LEN:
            self.buf.append(byte)
            self.data_len = self.frame_len - 2
            if self.data_len == 0:
                self.state = self.STATE_CRC
            else:
                self.state = self.STATE_DATA
        elif self.state == self.STATE_DATA:
            self.buf.append(byte)
            if len(self.buf) >= self.frame_len:
                self.state = self.STATE_CRC
        elif self.state == self.STATE_CRC:
            expected = crc8(self.buf)
            if byte == expected:
                self.state = self.STATE_EOF
            else:
                self.reset()
        elif self.state == self.STATE_EOF:
            self.reset()
            if byte == self.EOF:
                if len(self.buf) >= 2:
                    cmd = self.buf[1]
                    data = bytes(self.buf[2:]) if len(self.buf) > 2 else b""
                    return (self.frame_len, cmd, data)
        return None


# Auth algorithm (must match firmware FK_AuthComputeResponse)
SBOX = bytes([
    
])
KEY = bytes([0xAB, 0xCD, 0xEF, 0x01, 0x23, 0x45, 0x67, 0x89])

def compute_response(challenge):
    """Must match firmware FK_AuthComputeResponse exactly: XOR → SBOX → ROTL1 → SBOX+permute"""
    tmp = bytearray(8)
    for i in range(8):
        tmp[i] = challenge[i] ^ KEY[i]
    for i in range(8):
        tmp[i] = SBOX[tmp[i]]
    for i in range(1, 8):
        tmp[i] ^= ((tmp[i-1] << 1) | (tmp[i-1] >> 7)) & 0xFF
    resp = bytearray(8)
    for i in range(8):
        resp[i] = SBOX[tmp[(i + 3) % 8]]
    return bytes(resp)


# ====== Main test ======
PORT = "/dev/ttyUSB0"
BAUD = 115200

ser = serial.Serial(PORT, BAUD, timeout=3)
time.sleep(0.5)
ser.reset_input_buffer()

# Drain any stale boot data
ser.timeout = 0.3
stale = b""
while True:
    d = ser.read(4096)
    if not d:
        break
    stale += d
ser.timeout = 2

idx = 1  # global test counter

def recv_frame(timeout=3):
    """Read bytes until a complete valid frame is found"""
    parser = FrameParser()
    deadline = time.time() + timeout
    raw = b""
    while time.time() < deadline:
        byte_data = ser.read(1)
        if not byte_data:
            if raw:
                break
            time.sleep(0.05)
            continue
        raw += byte_data
        result = parser.feed(byte_data[0])
        if result is not None:
            return result  # (flen, cmd, data)
    return None

def send_and_recv(cmd, data=b"", fast=False):
    """Send frame and collect response"""
    frame = build_frame(cmd, data)
    ser.reset_input_buffer()
    ser.write(frame)
    if fast:
        time.sleep(0.01)
    else:
        time.sleep(0.15)
    return recv_frame(timeout=3)

def test_step(name, cmd, data=b"", expected_cmd=None):
    global idx
    f = send_and_recv(cmd, data)
    if f and f[1] == expected_cmd:
        print(f"  {idx}. {name}: PASS ✅")
        result = True
    elif f:
        print(f"  {idx}. {name}: FAIL (got cmd=0x{f[1]:02X}, expected 0x{expected_cmd:02X}, data={f[2].hex()})")
        result = False
    else:
        print(f"  {idx}. {name}: FAIL (no response)")
        result = False
    idx += 1
    return f, result


print("=" * 52)
print("FlashKey FK-01 握手功能测试")
print("=" * 52)

# 1. PING → PONG
print("\n--- 1. PING → PONG ---")
f, ok1 = test_step("PING", 0x01, b"", 0x02)
if not ok1:
    # Try with DTR reset
    print("  [重试] DTR复位后重试...")
    ser.dtr = False
    time.sleep(0.05)
    ser.dtr = True
    time.sleep(1.0)
    ser.reset_input_buffer()
    ser.timeout = 0.3
    ser.read(4096)  # drain boot msgs
    ser.timeout = 2
    time.sleep(0.2)
    f, ok1 = test_step("PING (after reset)", 0x01, b"", 0x02)

if not ok1:
    # Send repeated PINGs and look for any frame in collected data
    print("  [重试] 连续发送 10 次 PING...")
    parser = FrameParser()
    all_raw = b""
    for _ in range(10):
        ser.reset_input_buffer()
        ser.write(build_frame(0x01))
        time.sleep(0.05)
    time.sleep(0.5)
    ser.timeout = 0.01
    while True:
        d = ser.read(4096)
        if not d:
            break
        all_raw += d
    ser.timeout = 2
    for b in all_raw:
        result = parser.feed(b)
        if result is not None:
            f = result
            if f[1] == 0x02:
                print(f"  PING (burst): PASS ✅ (found PONG in {len(all_raw)} bytes)")
                ok1 = True
                break
    if not ok1:
        print(f"  PING (burst): FAIL (collected {len(all_raw)} bytes: {all_raw[:40].hex()})")

# 2. CHALLENGE → device computes response
print("\n--- 2. CHALLENGE → RESPONSE ---")
chal = os.urandom(8)
f = send_and_recv(0x10, chal)
if f and f[1] == 0x10:
    dev_resp = f[2]
    local_resp = compute_response(chal)
    match = dev_resp == local_resp
    print(f"  CHALLENGE: PASS ✅ (device returned response)")
    print(f"    Device: {dev_resp.hex()}")
    print(f"    Local:  {local_resp.hex()}")
    print(f"    Algorithm match: {'✅' if match else '❌ MISMATCH!'}")
    ok2 = True
else:
    print(f"  CHALLENGE: FAIL (got: cmd=0x{f[1]:02X} data={f[2].hex() if f else 'NONE'})")
    ok2 = False
    dev_resp = None

# 3. Correct response → AUTH_OK
print("\n--- 3. 正确密钥 → AUTH_OK(0x13) ---")
if ok2:
    time.sleep(0.1)
    f = send_and_recv(0x11, local_resp)
    if f and f[1] == 0x13:
        print(f"  AUTH_OK: PASS ✅ (cmd=0x13)")
        ok3 = True
    elif f:
        print(f"  AUTH_OK: FAIL (got cmd=0x{f[1]:02X}, expected 0x13, data={f[2].hex()})")
        ok3 = False
    else:
        print(f"  AUTH_OK: FAIL (no response)")
        ok3 = False
else:
    print(f"  AUTH_OK: SKIP (no challenge response)")
    ok3 = False

# 4. Wrong response → AUTH_FAIL
print("\n--- 4. 错误密钥 → AUTH_FAIL(0x14) ---")
time.sleep(0.1)
f = send_and_recv(0x11, b"\x00" * 8)
if f and f[1] == 0x14:
    print(f"  AUTH_FAIL: PASS ✅ (cmd=0x14)")
    ok4 = True
elif f:
    print(f"  AUTH_FAIL: FAIL (got cmd=0x{f[1]:02X}, expected 0x14, data={f[2].hex()})")
    ok4 = False
else:
    print(f"  AUTH_FAIL: FAIL (no response)")
    ok4 = False


ser.close()

# ====== Report ======
print("\n" + "=" * 52)
print("FlashKey FK-01 握手功能测试报告")
print("日期: 2026-06-11")
print("=" * 52)
print()
print("1. 环境")
print("   - 芯片: CH32V203C8T6")
print("   - 通道: /dev/ttyUSB0 @ 115200 (CH340C)")
import subprocess
commit = subprocess.run(["git", "log", "--oneline", "-1"], capture_output=True, text=True).stdout.strip()
print(f"   - 固件 commit: {commit}")
print("   - 测试脚本: test_s1_handshake.py (已修正 AUTH_OK=0x13, AUTH_FAIL=0x14)")
print()
print("2. 测试结果")
print(f"   {'✅' if ok1 else '❌'} PING → PONG: {'PASS' if ok1 else 'FAIL'}")
print(f"   {'✅' if ok2 else '❌'} CHALLENGE → RESPONSE: {'PASS' if ok2 else 'FAIL'}")
if ok2:
    print(f"   {'✅' if match else '❌'} Auth 算法一致性: {'PASS' if match else 'FAIL (local vs device mismatch)'} ")
print(f"   {'✅' if ok3 else '❌'} 正确密钥 → AUTH_OK(0x13): {'PASS' if ok3 else 'FAIL'}")
print(f"   {'✅' if ok4 else '❌'} 错误密钥 → AUTH_FAIL(0x14): {'PASS' if ok4 else 'FAIL'}")
print()
print("3. PB11 LED 状态: 未确认（需要实物观察）")
print("   - 初始状态: OFF (HIGH)")
print("   - 认证通过后: ON (LOW)")
print("   - 认证失败后: OFF (HIGH)")
print()
all_ok = ok1 and ok2 and ok3 and ok4 and match
print(f"4. 结论: {'✅ 全部PASS' if all_ok else '❌ 部分FAIL'}")
if not all_ok:
    fails = []
    if not ok1: fails.append("PING→PONG")
    if not ok2: fails.append("CHALLENGE")
    if not ok3: fails.append("AUTH_OK")
    if not ok4: fails.append("AUTH_FAIL")
    if ok2 and not match: fails.append("算法一致性")
    print(f"   失败项: {', '.join(fails)}")
print()
print("=" * 52)
